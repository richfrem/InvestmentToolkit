...
XML_DIR = REPO_ROOT / 'legacy-system' / 'oracle-forms' / 'XML'
OUT_DIR = REPO_ROOT / 'legacy-system' / 'reference-data' / 'inventories'
...
def extract_metadata(xml_path: Path, artifact_type: str) -> dict:
    ...
    metadata = {
        ...
        'path': str(xml_path.relative_to(REPO_ROOT)).replace('\\', '/'),
