...
def investigate_topic(topic):
    ...
    # We use cli.py init-context --type br --manifest [PATH]
    ...
    # cli.py init-context args: --target, --type.  Does it accept --manifest?
    # Checking cli.py: init-context uses default script path logic. It does NOT accept --manifest arg in the parser?
    # Wait, cli.py line 288: context_parser.add_argument("--target")...
    ...
    # Switch to 'python tools/cli.py manifest init' for custom path control.
    cmd_init = [
        sys.executable, str(project_root / "tools" / "cli.py"), 
        "manifest", "init",
