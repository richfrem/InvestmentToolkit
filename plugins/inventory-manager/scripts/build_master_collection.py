...
INVENTORIES = {
    "oracle_forms_manifest.json": "FORM",
...
OBJECT_TYPES = {
    "FORM": {
        ...
        "sourceCollection": "oracle_forms_manifest.json",
...
def populate_forms(objects):
    """Populate FORM objects from oracle_forms_manifest.json."""
    manifest_path = os.path.join(INVENTORIES_DIR, 'oracle_forms_manifest.json')
    try:
        ...
        for item in data if isinstance(data, list) else data.values():
            try:
                obj_id = item.get('ObjectID', '').upper()
                if not obj_id:
                    continue
                ...
                obj_data = {
                    "type": "FORM",
                    "name": item.get('ObjectName', obj_id),
                    "artifacts": artifacts
                }
