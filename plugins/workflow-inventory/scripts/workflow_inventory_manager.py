...
Usage:
    ...
    # Search workflows
    python3 plugins/workflow-inventory/scripts/workflow_inventory_manager.py --search "keyword"
...
OUTPUT_DIR = PROJECT_ROOT / "docs" / "antigravity" / "workflow"
...
def generate_json(workflows):
    ...
    data = {
        "metadata": {
            ...
            "count": len(workflows),
            "source": str(WORKFLOWS_DIR.relative_to(PROJECT_ROOT)).replace("\\", "/")
        },
        "workflows": workflows
    }
    save_inventory(data)
...
Examples:
  ...
  --show "codify-form"  Show details for a specific workflow
    ...
    if not any([args.scan, args.search, args.list, args.show]):
        print("No arguments provided. Usage: --scan | --list | --search | --show")
        parser.print_help()
        return
