...
try:
    ...
    # Add plugin path if not in sys.path
    project_root = Path(__file__).resolve().parent.parent.parent.parent
    plugin_path = project_root / "plugins" / "context-bundler" / "scripts"
    if str(plugin_path) not in sys.path:
        sys.path.append(str(plugin_path))
    try:
        from path_resolver import resolve_path
    except ImportError:
        print("CRITICAL: Could not import path_resolver.")
        sys.exit(1)
...
class WorkflowManager:
    ...
    def __init__(self):
        ...
        try:
            ...
            # self.investigate_utils = Path(resolve_path("tools/investigate/utils")) # DEPRECATED
            self.next_num_script = Path(resolve_path("plugins/adr-manager/scripts/next_number.py"))
