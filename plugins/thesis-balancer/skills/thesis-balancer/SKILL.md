...
description: >
  ...
  detect drift, rebalance holdings, or improve the thesis formula.
  Also trigger on /review-portfolio or /thesis-health.
  Integrates deeply with stock-valuation plugin: surfaces pillar-level
  conviction signals, valuation gaps, strategic conflicts, and formula
  improvement recommendations grounded in AI fair-value analysis.
...
## Phase 6: Thesis Formula Improvement Protocol *(New)*
> Invoke this phase when user requests formula review, or automatically when Thesis Formula Score < 70.

Using `references/strategic_review_prompt.md` with full valuation payload:

```bash
# Build the full strategic review payload
python3 << 'EOF'
import json

review_payload = {
    "thesis": { "name": thesis_name, "pillars": pillars, "version": version },
    "pillarConvictionAudit": pillar_audit_results,   # from Phase 3E
    "strategicConflicts": conflicts,                  # from Phase 3B
    "valuationGapRanking": gap_ranking,               # from Phase 3F
    "thesisFormulaSCore": formula_score,              # from Phase 3E
    "holdingValuations": valuations,                  # from Phase 2
    "missingValuations": missing_tickers,             # from Phase 3D
}
print(json.dumps(review_payload, indent=2))
EOF
```

Then submit to the LLM using `references/strategic_review_prompt.md` as the system prompt.

**Expected output** (in addition to standard JSON): An `"formulaImprovements"` block:
```json
{
  "formulaImprovements": [
    {
      "pillar": "compute",
      "finding": "INTC is 10.87% target with −77% FV gap. NVDA at 4.32% target has +124% upside.",
      "recommendation": "Reduce INTC target weight from 10.87% to 3-5%. Increase NVDA from 4.32% to 8-10%.",
      "rationale": "Valuation evidence does not support the Sovereign Foundry bet at current sizing."
    }
  ]
}
```

Present formula improvements conversationally, then ask for confirmation before applying any thesis weight changes.
...
- Strategic Review Prompt: [✅ references/strategic_review_prompt.md / ❌ Not used]
