...
- All secrets (refresh tokens, API keys) are prioritized as system environment variables (e.g., `~/.zshrc`).
- Active rotation tokens are stored in a local, git-ignored `.questrade_cache` file (see [ADR 015](015-stateful-token-rotation.md)).
- Local `.env` files are permitted but checked second and must never be committed.
