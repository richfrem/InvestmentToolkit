# Wave 3 — CAD Exchange-Rate Retained-JSON Rationale Bar (open item for the Task 9 exit report)
...
| User-approved exception? | **NOT YET — pending explicit approval, named here for the exit report.** This is a real design decision (new schema for per-currency broker totals) that must be surfaced to the user in the Task 9 exit report, not decided unilaterally in this dispatch. |
