...
+
+**31. Single-Repo Confinement & Dangerous Cross-Repo Assumptions**: `agent-plugins-skills` is a separate ecosystem repository. Financial and toolkit plugins (`portfolio-advisor`, `stock-valuation`, `tradingview`, etc.) exist exclusively in `InvestmentToolkit`. Never touch, branch, or push changes into `agent-plugins-skills` unless explicitly ordered. Always pull `origin/main` before creating any worktree or branch.
