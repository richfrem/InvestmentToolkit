...
with Phase 4's E3 prediction_ledger.load_predictions() to link an existing
alert's metadata to a claim, for audit ("was this alert based on this
claim?").
...
investment_screener/backend/data/alerts_state.jsonl path. `load_predictions`
is monkeypatched directly on the alert_manager module (its imported
reference) to a stub returning a fixture claims list — no test reads or
writes the real, tracked data/predictions.jsonl.
"""
...
def test_link_alert_to_claim_returns_true_and_persists_link(alerts_state_path, monkeypatch):
    ...
    monkeypatch.setattr(
        alert_manager, "load_predictions", lambda: [_make_claim(id="claim-abc")]
    )
...
def test_link_alert_to_claim_returns_false_when_claim_not_found(alerts_state_path, monkeypatch):
    ...
    save_alert_metadata(_make_record(id="alert-123"))
    monkeypatch.setattr(
        alert_manager, "load_predictions", lambda: [_make_claim(id="some-other-claim")]
    )
