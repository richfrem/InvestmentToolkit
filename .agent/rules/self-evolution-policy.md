...
12. **Modular Validation Functions for Every UI Analytical Metric**: Every metric rendered in the stock analysis modals, technical cards, and DCF projection tabs MUST be computed and validated by discrete, test-backed functions in versioned CLI tooling (e.g. `validate_stock_metrics.py`, `stock_intake_persist.py`). When a UI calculation error or formatting anomaly occurs, agents must repair the underlying modular calculation function and its unit test rather than manually overwriting values in ad-hoc payloads.
