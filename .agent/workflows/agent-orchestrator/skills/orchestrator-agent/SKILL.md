...
**MANDATORY PRE-REQUISITE**: You *must* create the worktree first.
```bash
spec-kitty implement <WP-ID>  # Creates .worktrees/<WP-ID>
/agent-orchestrator:delegate  # Generates handoff packet
```
...
### 6. Closure (Feature Completion - Deterministic Protocol)

> **THE OUTER LOOP OWNS CLOSURE.**
> After the Inner Loop finishes coding, the orchestrator MUST drive the full chain.
...
# E. Verification & Intel Sync
git worktree list            # Verify cleanup
/rlm-factory_distill         # Update RLM
/inventory-manager_curate-inventories # Rebuild Inventory
```
