...
### 1.5. Worktree Creation (MANDATORY)
Before starting ANY work package, you MUST create the isolated environment:

- **Command:** `spec-kitty implement WP-xx`
- **System Actions:**
    - Creates `.worktrees/WP-xx` (Clean workspace)
    - Checks out feature branch `WP-xx`
- **User Action:** `cd .worktrees/WP-xx` (CRITICAL)
...
### 3. Feature Completion (Deterministic Closure Protocol)
Once **ALL** WPs are in the `done` column, execute the **MANDATORY** 6-step closure chain:

**Chain:** Review → Accept → Retrospective → Merge → Verify → Intel Sync

1.  **Acceptance:**
    - **Command:** `spec-kitty accept`
    - *System Actions:* Validation of all tasks and spec requirements.

2.  **Retrospective (MANDATORY):**
    - **Command:** `/spec-kitty_retrospective`
    - *System Actions:* Generates retro artifact in `kitty-specs/`. **Merge will fail without this.**

3.  **Closure Merge:**
    - **Command:** `spec-kitty merge`
    - *Context:* **Main Repo Root** (Never inside a worktree)
    - *System Actions:*
        - Merges all WPs to local `main`
        - **Auto-Deletes** worktrees and branches
        - Squashes commits for clean history

4.  **Post-Merge Verification:**
    - **Command:** `git log` + `git worktree list` + `git branch`
    - *Goal:* Verify no orphaned resources remain.

5.  **Intelligence Sync:**
    - **Command:** `/rlm-factory_distill` + `/vector-db_ingest`
    - *Goal:* Update agent memory with new capabilities.
