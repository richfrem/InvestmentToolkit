...
## 0.5. Create Worktree (MANDATORY for Clean Merge)
...
```bash
spec-kitty implement WP-xx
```

**What this does:**
1. Creates `.worktrees/WP-xx` directory
2. Checks out a feature branch for this work package
3. Sets up isolated workspace for implementation

**After running:**
- **CRITICAL:** You MUST `cd .worktrees/WP-xx` immediately.
- All implementation happens here.
- When done, `spec-kitty merge` will clean this up automatically.
...
### Step 4i: Inventory Rebuild (System Consistency)
```bash
/inventory-manager_curate-inventories
```
**PROOF**: Paste output confirming master_object_collection.json updated.
