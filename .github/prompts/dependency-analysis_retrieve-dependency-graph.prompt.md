...
```bash
python tools/cli.py dependencies --target [TargetID]
**Command (Deep Search):**
```bash
python tools/cli.py dependencies --target [TargetID] --deep
**Command (JSON Output):**
```bash
python tools/cli.py dependencies --target [TargetID] --json
```
// turbo