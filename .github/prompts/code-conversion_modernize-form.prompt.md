...
> **ADR Reference:** [ADR-0019: Reusable Investigate-Codify-Modernize Pattern](../../docs/ADRs/0019-reusable-investigate-codify-modernize-pattern.md)
> 
...
```bash
python tools/cli.py init-context --target [FormID] --type form
```
...
**What this does:**
1. Initializes context (`cli.py init-context --target [FormID] --type form`)
...
```bash
view_file docs/architecture/UI_Role_Based_Configuration_Strategy.md
```

### Step 5: Generate UI Config
```bash
python plugins/code-conversion/scripts/generate_ui_config.py --target [FormID]
```
...
### Step 10: Create Form Page Component
...
- **Logic**: Map Oracle blocks to React state/Grid components
...
```bash
python tools/cli.py rules register --source [FormID] --description "Title" --priority P2
```

### Step 18: Post-Processing Pipeline
```bash
python plugins/link-checker/scripts/enrich_links_v2.py --file [OverviewFile]
/codify-rlm-distill [OverviewFile]
python plugins/legacy-doc-gen/scripts/update_analysis_tracking.py [FormID] --notes "Modernized to React"
```
...
1. ✅ **Running React app** at http://localhost:3001/
