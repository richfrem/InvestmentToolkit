...
Returns the next available number for any artifact type with sequential IDs.

Usage:
    python next_number.py --type spec        # Next spec: 002
    python next_number.py --type task        # Next maintenance task: 0045
    python next_number.py --type br          # Next business rule: BR-0009
    python next_number.py --type bw          # Next workflow: BW-0003
    python next_number.py --type all         # Show all types
"""
import os
import re
import sys
...
ARTIFACT_TYPES = {
    ...
    "task": {
        ...
        "format": "{:04d}",
        "prefix": "",
        "search_subdirs": True  # Search backlog, in-progress, done
    },
    "spec": {
        "name": "Specification",
        "directory": "specs",
...
def find_max_number(artifact_type: str, project_root: Path) -> int:
    ...
    if config.get("search_subdirs"):
        dirs_to_search = [base_dir / "backlog", base_dir / "todo", base_dir / "in-progress", base_dir / "done"]
    else:
...
def main():
    ...
    parser.add_argument('--pattern', help='Regex pattern for --dir (default: (\d{4}))', default=r'(\d{4})')
