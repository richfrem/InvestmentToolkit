...
def main():
    ...
    run_command([python_exec, "-m", "pip", "install", "--upgrade", "pip"], shell=True, env=env)
    run_command([python_exec, "-m", "pip", "install", "yfinance", "pandas", "uvicorn", "fastapi"], shell=True, env=env)

    # 7. Install Node dependencies
    Colors.print("Installing Node dependencies...", Colors.GREEN)
    run_command(["npm", "install"], shell=True, env=env)

    # 8. Build Backend
    Colors.print("Building Backend...", Colors.GREEN)
    run_command(["npm", "run", "build", "-w", "backend"], shell=True, env=env)
