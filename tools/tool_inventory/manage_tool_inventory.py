...
Usage Examples:
    python tools/tool-inventory/manage_tool_inventory.py --help
    python tools/tool-inventory/manage_tool_inventory.py list
    python tools/tool-inventory/manage_tool_inventory.py search "keyword"
    python tools/tool-inventory/manage_tool_inventory.py remove --path "path/to/tool.py"
    python tools/tool-inventory/manage_tool_inventory.py update --path "tool.py" --desc "New description"
    python tools/tool-inventory/manage_tool_inventory.py discover --auto-stub
    python tools/tool-inventory/manage_tool_inventory.py summarize-missing
    python tools/tool-inventory/manage_tool_inventory.py sync-from-cache
    python tools/tool-inventory/manage_tool_inventory.py reset-from-cache
    python tools/tool-inventory/manage_tool_inventory.py clear-inventory
...
class InventoryManager:
    ...
    def _trigger_distillation(self, tool_path: str):
        ...
        """
        distiller_script = self.root_dir / "tools/tool-inventory/distiller.py"
    ...
    def _remove_from_cache(self, tool_path: str):
        ...
        try:
            ...
            if removed:
                print(f"✅ Synced removal with RLM Cache.")
            else:
                pass # cleanup_cache already prints warnings
                
        except ImportError:
            print(f"⚠️ Could not import tools.curate.rlm.cleanup_cache. Cache may be out of sync.")
        except Exception as e:
            print(f"⚠️ Error syncing with cache: {e}")
